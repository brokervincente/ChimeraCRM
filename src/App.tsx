...
export default App;// Piani Provvigionali Content
const PianiProvvigionaliContent = () => {
  ...
  const aggiornaPercentuale = (tipoLivello: string, livelloId: number, colonnaIndex: number, valore: string) => {
    ...
    setValoriPercentuali({...valoriPercentuali, [key// Fornitori Content
const FornitoriContent = () => {
  ...
  return (
    <div className="space-y-6">
      ...
      {/* Modal Configurazione Fornitore - Breve esempio */}
        ...
        <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50">
          <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
            ...
            <div className="p-6 space-y-8">
              {/* Configurazione semplificata per evitare codice troppo lungo */}
              <div className="bg-blue-50 border border-blue-200 rounded-lg p-4">
                <h4 className="text-lg font-medium text-blue-900 mb-4">Configurazioni Base</h4>
                <p className="text-sm text-blue-700">
                  Qui verranno implementate tutte le configurazioni avanzate per il fornitore
                  (ritardi pagamento, logiche storno, mesi parziali, ecc.)
                </p>
              </div>
            </div>
      ...
      <div className="bg-white rounded-lg shadow-xl max-w-4xl w-full mx-4 max-h-[90vh] overflow-y-auto">
        ...
        <div className="bg-white p-6 rounded-lg shadow-sm border border-gray-200">
          <div className="flex items-center">
            ...
            <div className="ml-4">import { useState } from 'react';
import { 
...
const App = () => {
  ...
  const menuStructure = [
    ...
    },
    {
      ...
      icon: Settings,
      items: [
        { id: 'profilo', title: 'Profilo', status: 'complete' },
...
// Profilo Content
const ProfiloContent = () => {
  const [profiloData, setProfiloData] = useState({
    // Dati Agenzia
    nomeAgenzia: 'CRM Energia Consulting',
    piva: '12345678901',
    ...
    fax: '+39 02 7654321',
    email: 'info@crmenergiaconsulting.it',
    pec: 'pec@crmenergiaconsulting.pec.it',
    sitoWeb: 'www.crmenergiaconsulting.it',
    ...
    emailResponsabile: 'mario.rossi@crmenergiaconsulting.it',
    telefonoResponsabile: '+39 333 9876543',
    
    // Impostazioni Business
    numeroLicenza: 'CRM-ENE-2024-001',
    dataScadenzaLicenza: '2024-12-31',
    tipoAccount: 'Premium',
    numeroMaxCollaboratori: 50,
    ...
    notificheSms: false,
    notificheAlert: true,
    frequenzaBackup: 'giornaliero'
  });

  const [logoFile, setLogoFile] = useState<File | null>(null);
  const [logoPreview, setLogoPreview] = useState<string>('');
  const [cambioPasswordAttivo, setCambioPasswordAttivo] = useState(false);

  const handleInputChange = (campo: string, valore: string | boolean) => {
    setProfiloData(prev => ({
      ...prev,
      [campo]: valore
    }));
  ...
  const salvaImpostazioni = () => {
    // Validazioni
    if (cambioPasswordAttivo) {
      if (profiloData.nuovaPassword !== profiloData.confermaPassword) {
        alert('Le password non corrispondono');
        return;
      }
      if (profiloData.nuovaPassword.length < 6) {
        alert('La password deve avere almeno 6 caratteri');
        return;
      }
    }

    console.log('Salvataggio profilo:', profiloData);
    console.log('Logo file:', logoFile);
    
    // Qui andrebbe implementato il salvataggio effettivo
  ...
  return (
    <div className="space-y-6">
      <div className="flex justify-between items-center">
        <div>
          <h3 className="text-2xl font-semibold text-gray-900">Profilo Utente</h3>
          <p className="text-sm text-gray-600 mt-1">
            Gestisci le informazioni del tuo account e dell'agenzia
          </p>
        </div>
        <button 
          onClick={salvaImpostazioni}
      ...
      {/* Logo e Account Info */}
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        <h4 className="text-lg font-medium text-gray-900 mb-4">Logo e Account</h4>
        <div className="flex items-start space-x-6">
          <div className="flex flex-col items-center space-y-3">
            <div className="w-24 h-24 bg-gray-200 rounded-lg border-2 border-dashed border-gray-300 flex items-center justify-center overflow-hidden">
              {logoPreview ? (
          ...
          <div className="flex-1 grid grid-cols-1 md:grid-cols-2 gap-4">
            <div>
              ...
              <input
                type="text"
                value={profiloData.tipoAccount}
                readOnly
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-600"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Numero Licenza</label>
              <input
                type="text"
                value={profiloData.numeroLicenza}
                readOnly
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-600"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Scadenza Licenza</label>
              <input
                type="date"
                value={profiloData.dataScadenzaLicenza}
                readOnly
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-600"
              />
            </div>
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Max Collaboratori</label>
              <input
                type="number"
                value={profiloData.numeroMaxCollaboratori}
                readOnly
                className="w-full px-3 py-2 border border-gray-300 rounded-md bg-gray-50 text-gray-600"
              />
            </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          <div className="md:col-span-2">
            <label className="block text-sm font-medium text-gray-700 mb-2">Nome Agenzia *</label>
            <input
              type="text"
              value={profiloData.nomeAgenzia}
              onChange={(e) => handleInputChange('nomeAgenzia', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
          ...
          <div className="md:col-span-2">
            ...
            <input
              type="text"
              value={profiloData.codiceFiscale}
        ...
        {/* Indirizzo */}
        <div className="mt-6">
          <h5 className="text-md font-medium text-gray-800 mb-3">Indirizzo Sede</h5>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
            <div className="lg:col-span-2">
              <label className="block text-sm font-medium text-gray-700 mb-2">Via/Indirizzo *</label>
              <input
                type="text"
                value={profiloData.indirizzo}
        ...
        {/* Contatti */}
        <div className="mt-6">
          <h5 className="text-md font-medium text-gray-800 mb-3">Contatti</h5>
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            <div>
            ...
            <div>
              <label className="block text-sm font-medium text-gray-700 mb-2">Cellulare *</label>
              <input
                type="tel"
                value={profiloData.cellulare}
            ...
            <div className="md:col-span-2">
              ...
              <input
                type="email"
                value={profiloData.pec}
                onChange={(e) => handleInputChange('pec', e.target.value)}
                className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                placeholder="pec@esempio.pec.it"
              />
            </div>
          </div>
        </div>
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            ...
            <input
              type="text"
              value={profiloData.nomeResponsabile}
              onChange={(e) => handleInputChange('nomeResponsabile', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="Mario"
            />
          </div>
          <div>
            ...
            <input
              type="text"
              value={profiloData.cognomeResponsabile}
              onChange={(e) => handleInputChange('cognomeResponsabile', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="Rossi"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Email Personale</label>
            <input
              type="email"
              value={profiloData.emailResponsabile}
              onChange={(e) => handleInputChange('emailResponsabile', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
              placeholder="mario.rossi@esempio.it"
            />
          </div>
          <div>
            <label className="block text-sm font-medium text-gray-700 mb-2">Telefono Personale</label>
            <input
              type="tel"
              value={profiloData.telefonoResponsabile}
              onChange={(e) => handleInputChange('telefonoResponsabile', e.target.value)}
              className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
            ...
            <div className="flex items-end">
              <button
                onClick={() => setCambioPasswordAttivo(!cambioPasswordAttivo)}
                className={`px-4 py-2 rounded-lg transition-colors ${
                  cambioPasswordAttivo 
                    ? 'bg-red-600 text-white hover:bg-red-700' 
                    : 'bg-blue-600 text-white hover:bg-blue-700'
                }`}
              >
                {cambioPasswordAttivo ? 'Annulla Cambio Password' : 'Cambia Password'}
              </button>
            </div>
          </div>
          ...
          {cambioPasswordAttivo && (
            <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 space-y-4">
              <h5 className="text-md font-medium text-yellow-800">Cambio Password</h5>
              <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-2">Password Attuale *</label>
                  <input
                    type="password"
                    value={profiloData.passwordAttuale}
                    onChange={(e) => handleInputChange('passwordAttuale', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="••••••••"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="password"
                    value={profiloData.nuovaPassword}
                    onChange={(e) => handleInputChange('nuovaPassword', e.target.value)}
                    className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                    placeholder="••••••••"
                  />
                </div>
                <div>
                  ...
                  <input
                    type="password"
                    value={profiloData.confermaPassword}
      ...
      <div className="bg-white rounded-lg shadow-sm border border-gray-200 p-6">
        ...
        <div className="space-y-4">
          <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
            <div>
              <h5 className="text-md font-medium text-gray-800 mb-3">Notifiche</h5>
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-700">Notifiche Email</span>
                  <input
                    type="checkbox"
                    checked={profiloData.notificheEmail}
                    onChange={(e) => handleInputChange('notificheEmail', e.target.checked)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                </div>
                <div className="flex items-center justify-between">
                  ...
                  <input
                    type="checkbox"
                    checked={profiloData.notificheSms}
                    onChange={(e) => handleInputChange('notificheSms', e.target.checked)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                </div>
                <div className="flex items-center justify-between">
                  <span className="text-sm text-gray-700">Alert Sistema</span>
                  <input
                    type="checkbox"
                    checked={profiloData.notificheAlert}
                    onChange={(e) => handleInputChange('notificheAlert', e.target.checked)}
                    className="h-4 w-4 text-blue-600 focus:ring-blue-500 border-gray-300 rounded"
                  />
                </div>
              </div>
            ...
            <div>
              <h5 className="text-md font-medium text-gray-800 mb-3">Backup</h5>
              <div>
                <label className="block text-sm font-medium text-gray-700 mb-2">Frequenza Backup</label>
                <select
                  value={profiloData.frequenzaBackup}
                  onChange={(e) => handleInputChange('frequenzaBackup', e.target.value)}
                  className="w-full px-3 py-2 border border-gray-300 rounded-md focus:ring-2 focus:ring-blue-500 focus:border-blue-500"
                >
                  <option value="giornaliero">Giornaliero</option>
                  <option value="settimanale">Settimanale</option>
                  <option value="mensile">Mensile</option>
                  <option value="manuale">Solo Manuale</option>
                </select>
              </div>
            </div>
          </div>
      ...
      <div className="bg-blue-50 border border-blue-200 rounded-lg p-6">
        <div className="flex items-start">
          <AlertCircle className="h-5 w-5 text-blue-600 mt-0.5 flex-shrink-0" />
          <div className="ml-3">
            <h5 className="text-sm font-medium text-blue-800 mb-2">Informazioni Importanti</h5>
            <ul className="text-sm text-blue-700 space-y-1">
              <li>• I campi contrassegnati con * sono obbligatori</li>
              <li>• Il logo viene visualizzato nell'header al posto dell'icona utente</li>
              <li>• Le modifiche alla password richiedono la password attuale</li>
              <li>• I backup automatici vengono eseguiti secondo la frequenza impostata</li>
              <li>• Per assistenza tecnica contattare il supporto CRM Energia</li>
            </ul>
          </div>
        </div>
      </div>
